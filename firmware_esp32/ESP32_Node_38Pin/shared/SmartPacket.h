...
// Compact binary encoding for telemetry data.
// After decryption, payload is:
//   [num_readings:1B] [pin:1B] [type:1B] [value:2B] ...
// value is int16_t scaled by 10 (25.3 -> 253).
// Special sentinel values for error states.
...
constexpr uint8_t  BIN_HEADER_SIZE  = 1;       // count byte only (priority/report_mode in header, not payload)
...
// Dynamic intervals (ms) based on priority
constexpr unsigned long INTERVAL_NORMAL_MS   = 90000UL;   // 90s normal
constexpr unsigned long INTERVAL_ABNORMAL_MS = 45000UL;   // 45s abnormal
constexpr unsigned long INTERVAL_CRITICAL_MS = 10000UL;   // 10s critical
...
// Check if decrypted payload is binary format (first byte is a count 0-24)
// vs text format (starts with printable ASCII like 'a' for "auth=" or 'c' for "cmd=").
...
// ---- Field parser for "key=value&key2=value2" strings ----
