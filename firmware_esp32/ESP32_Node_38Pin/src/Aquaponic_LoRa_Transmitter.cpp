...
#ifndef NFC_ONLY_MODE
#include <WiFi.h>
#include <WebServer.h>
#endif

#ifdef NFC_CONFIG_ENABLED
...
// NFC_ONLY_MODE: No WiFi AP - configuration via NFC only
...
constexpr size_t LORA_MAX_PACKET_BYTES = 255;
constexpr size_t SMART_PACKET_OVERHEAD_BYTES = sizeof(SmartPacket::Header) + SmartPacket::NONCE_SIZE + sizeof(uint32_t);
constexpr size_t MAX_PLAIN_PAYLOAD_BYTES = LORA_MAX_PACKET_BYTES - SMART_PACKET_OVERHEAD_BYTES;
...
String buildTelemetryPayloadChunk(
    uint32_t sequence,
    uint8_t priority,
    uint8_t reportMode,
    size_t &segmentIndex,
    uint16_t packetPart) {
  String payload = buildPlainPayload(sequence, priority, reportMode);
  payload += "&packet_part=" + String(packetPart);
  payload += "&";
  const size_t startIndex = segmentIndex;

  while (segmentIndex < latestSegments.size()) {
    String nextSegment = latestSegments[segmentIndex].segment + "&";
    if (payload.length() + nextSegment.length() > MAX_PLAIN_PAYLOAD_BYTES && segmentIndex > startIndex) {
      break;
    }
    payload += nextSegment;
    segmentIndex++;
...
void sendCurrentTelemetry(bool forceImmediate) {
  ...
  uint16_t packetPart = 1;
  bool allChunksSent = true;

  while (segmentIndex < latestSegments.size()) {
    uint32_t sequence = packetSeq++;
    String plainText = buildTelemetryPayloadChunk(
        sequence,
        currentPriority,
        currentReportMode,
        segmentIndex,
    ...
    if (sendBinaryPacket(packetBytes)) {
      ...
      Serial.println("Packet chunk sent securely.");
      Serial.println("=======================================\n");
    } else {
      allChunksSent = false;
      Serial.println("LoRa send failed. Queueing packet chunk.");
...
#ifdef NFC_CONFIG_ENABLED
  // Initialize NFC for configuration (WiFi AP disabled)
  Serial.println("Initializing NFC config reader...");
  nfcReady = nfcReader.begin();
  if (nfcReady) {
    Serial.println("NFC reader ready - tap NFC tag to configure");
  } else {
    Serial.println("NFC init failed: " + nfcReader.getLastError());
  }
#else
...
void loop() {
#ifdef NFC_CONFIG_ENABLED
  // NFC config mode - check for NFC tag periodically
  static unsigned long lastNfcCheckMs = 0;
  if (nfcReady && millis() - lastNfcCheckMs >= 3000) {
    lastNfcCheckMs = millis();
    if (nfcReader.isCardPresent()) {
      Serial.println("NFC card detected - reading config...");
      String configJson, authKey;
      if (nfcReader.readConfig(configJson, authKey)) {
        Serial.println("NFC config read successfully");
        // Save config to flash
        prefs.begin(configNamespace, false);
        prefs.putString(configJsonPrefKey, configJson);
        prefs.putString(nodeSecurityPrefKey, authKey);
        prefs.end();
        ...
        if (error) {
          ...
          if (doc.containsKey("config")) {
            loadRuntimeSettingsFromDocument(doc);
            applyConfig(doc["config"]);
            Serial.println("NFC config applied");
          }
        }
      } else {
        Serial.println("NFC read failed: " + nfcReader.getLastError());
      }
    }
  }
#else
...
void handleSerialCommand(String input) {
...
#ifdef NFC_CONFIG_ENABLED
    Serial.println("NFCREAD        - Read config from NFC tag");
#endif
    Serial.println("");
  ...
  if (command == "NFCREAD") {
    ...
    String configJson, authKey;
    if (nfcReader.waitForCard(10000)) {
      if (nfcReader.readConfig(configJson, authKey)) {
        Serial.println("NFC config read successfully");
        prefs.begin(configNamespace, false);
        prefs.putString(configJsonPrefKey, configJson);
