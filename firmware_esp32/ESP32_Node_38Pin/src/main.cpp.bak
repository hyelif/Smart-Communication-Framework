...
static void setupDefaultSensors() {
    ...
    gSensorMgr.addSensor(17, "AI", "DHT22",     "Air Temperature/Humidity");
...
// Encodes sensor readings into compact binary format:
//   [count:1B] [pin:1B] [type:1B] [value:2B] ...
// value is int16_t scaled by 10.
...
// Listen-before-talk: sample RSSI to check if LoRa channel is busy.
// Returns true if channel is clear enough to transmit.
...
static void updateAdr() {
    ...
    if (newSf != gCurrentSf) {
        gCurrentSf = newSf;
        // NOTE: ADR calculation is kept for diagnostics, but SF switching is
        // disabled because the HQ receiver is fixed at SF9. Both radios must
        // use the same SF to communicate. If SF changes are needed in the
        // future, the HQ must be updated to match dynamically.
...
static bool checkLoRaResponse() {
    ...
    if (cmd == "relay") {
        ...
        if (relayAuth == gAuthKey) {
            ...
            // Map relay IDs to actual GPIO pins
            int relayPins[] = {26, 25};  // relay0=GPIO26, relay1=GPIO25
        ...
        return true;  // Mark as handled (prevents ACK timeout)
    }
...
// ================= IDLE WAIT =================
// Non-blocking wait that checks serial input.
...
void loop() {
    ...
    if (gNodeState == STATE_CONNECTED) {
        ...
        if (now - gLastSampleMs >= SmartPacket::SAMPLE_INTERVAL_MS) {
            gLastSampleMs = now;
            ...
            buildBinaryPayload(gReadings, gBinaryPayload);
            ...
            Serial.print("  (+"); Serial.print(SmartPacket::SAMPLE_INTERVAL_MS / 1000); Serial.println("s)");
        ...
        if (gNodeState == STATE_CONNECTED) {
            ...
            if (elapsed < SmartPacket::SAMPLE_INTERVAL_MS) {
                idleWait(SmartPacket::SAMPLE_INTERVAL_MS - elapsed);
            }
        }
    }
